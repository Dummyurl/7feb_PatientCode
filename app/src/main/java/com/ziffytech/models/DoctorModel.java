package com.ziffytech.models;

/**
 * Created by subhashsanghani on 5/22/17.
 */

public class DoctorModel
{
    private String doct_id;
    private String bus_id;
    private String doct_name;
    private String doct_degree;
    private String doct_phone;
    private String doct_email;
    private String doct_speciality;
    private String doct_photo;
    private String doct_about;
    private String bus_google_street;

    private String consult_from;
    private String bus_latitude;
    private String bus_longitude;
    private String working_day;
    private String doct_experience;
    private String rating;
    private String consult_fee;

    private String bus_title;
    private String bus_logo;
    private String start_con_time;
    private String end_con_time;
    private String is_ziffydoc ;

    private String distance;

    public String getDistance() {
        return distance;
    }

    public void setDistance(String distance) {
        this.distance = distance;
    }

    public String getConsult_from() {
        return consult_from;
    }

    public void setConsult_from(String consult_from) {
        this.consult_from = consult_from;
    }




    public String getRating() {
        return rating;
    }

    public void setRating(String rating) {
        this.rating = rating;
    }



    public String getConsult_fee() {
        return consult_fee;
    }

    public void setConsult_fee(String consult_fee) {
        this.consult_fee = consult_fee;
    }



    public String getDoct_experience() {
        return doct_experience;
    }

    public void setDoct_experience(String doct_experience) {
        this.doct_experience = doct_experience;
    }


    public String getWorking_day() {
        return working_day;
    }

    public void setWorking_day(String working_day) {
        this.working_day = working_day;
    }





    public String getBus_google_street() {
        return bus_google_street;
    }

    public void setBus_google_street(String bus_google_street) {
        this.bus_google_street = bus_google_street;
    }

    public String getBus_latitude() {
        return bus_latitude;
    }

    public void setBus_latitude(String bus_latitude) {
        this.bus_latitude = bus_latitude;
    }

    public String getBus_longitude() {
        return bus_longitude;
    }

    public void setBus_longitude(String bus_longitude) {
        this.bus_longitude = bus_longitude;
    }



    public String getBus_title() {
        return bus_title;
    }

    public void setBus_title(String bus_title) {
        this.bus_title = bus_title;
    }

    public String getBus_logo() {
        return bus_logo;
    }

    public void setBus_logo(String bus_logo) {
        this.bus_logo = bus_logo;
    }

    public String getStart_con_time() {
        return start_con_time;
    }

    public void setStart_con_time(String start_con_time) {
        this.start_con_time = start_con_time;
    }

    public String getEnd_con_time() {
        return end_con_time;
    }

    public void setEnd_con_time(String end_con_time) {
        this.end_con_time = end_con_time;
    }


    public String getDoct_id() {
        return doct_id;
    }

    public void setDoct_id(String doct_id) {
        this.doct_id = doct_id;
    }

    public String getBus_id() {
        return bus_id;
    }

    public void setBus_id(String bus_id) {
        this.bus_id = bus_id;
    }

    public String getDoct_name() {
        return doct_name;
    }

    public void setDoct_name(String doct_name) {
        this.doct_name = doct_name;
    }

    public String getDoct_degree() {
        return doct_degree;
    }

    public void setDoct_degree(String doct_degree) {
        this.doct_degree = doct_degree;
    }

    public String getDoct_phone() {
        return doct_phone;
    }

    public void setDoct_phone(String doct_phone) {
        this.doct_phone = doct_phone;
    }

    public String getDoct_email() {
        return doct_email;
    }

    public void setDoct_email(String doct_email) {
        this.doct_email = doct_email;
    }

    public String getDoct_speciality() {
        return doct_speciality;
    }

    public void setDoct_speciality(String doct_speciality) {
        this.doct_speciality = doct_speciality;
    }

    public String getDoct_photo() {
        return doct_photo;
    }

    public void setDoct_photo(String doct_photo) {
        this.doct_photo = doct_photo;
    }

    public String getDoct_about() {
        return doct_about;
    }

    public void setDoct_about(String doct_about) {
        this.doct_about = doct_about;
    }

    public String getIs_ziffydoc() {
        return is_ziffydoc;
    }

    public void setIs_ziffydoc(String is_ziffydoc) {
        this.is_ziffydoc = is_ziffydoc;
    }
}
